...
export async function loader() {
    ...
    const annualTotalAmount = transactions.filter(transaction => transaction.paidYear === String(currentYear)).map(transaction => transaction.amount).reduce((prev, current) => prev + current, 0);

    const monthlyTotalAmount = transactions.filter(transaction => (transaction.paidYear === String(currentYear)) && (transaction.paidMonth === String(currentMonth))).map(transaction => transaction.amount).reduce((prev, current) => prev + current, 0);
